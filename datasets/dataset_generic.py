...
class Generic_WSI_Classification_Dataset(Dataset):
    ...
    def get_split_from_df(self, all_splits, split_key='train'):
        ...
        if len(split) > 0:
            ...
            split = Generic_Split(df_slice, data_dir=self.data_dir, concat_features=self.concat_features, num_classes=self.num_classes)
        else:
            split = None
    ...
    def return_splits(self, from_id=True, csv_path=None):
        ...
        if from_id:
            if len(self.train_ids) > 0:
                ...
                train_split = Generic_Split(train_data, data_dir=self.data_dir, concat_features=self.concat_features, num_classes=self.num_classes)
            ...
            if len(self.val_ids) > 0:
                ...
                val_split = Generic_Split(val_data, data_dir=self.data_dir, concat_features=self.concat_features, num_classes=self.num_classes)
            ...
            if len(self.test_ids) > 0:
                ...
                test_split = Generic_Split(test_data, data_dir=self.data_dir, concat_features=self.concat_features, num_classes=self.num_classes)
...
class Generic_Split(Generic_MIL_Dataset):
    def __init__(self, slide_data, concat_features=False, data_dir=None, num_classes=2):
        self.use_h5 = False
        self.slide_data = slide_data
        self.data_dir = data_dir
        self.concat_features = concat_features
...
class Dataset_from_Split(Generic_Split):
    ...
    def __getitem__(self, index):
        ...
        find = False
        for data_dir_ in self.data_dir:
            if os.path.isfile(os.path.join(data_dir_, ID.split(":")[0]+'.h5')):
                find = True
                break
        if not find:
            raise FileNotFoundError("Please check the data_dir!")
            
        with h5py.File(os.path.join(data_dir_, ID.split(":")[0]+'.h5'),'r') as hdf5_file:
