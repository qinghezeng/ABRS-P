...
def seg_and_patch(source, save_dir, patch_save_dir, mask_save_dir, stitch_save_dir, annotation_dir, 
                  ...
                  skip_non_slide=False, slide_ext=None):
    
    slides = sorted(os.listdir(source))

    if skip_non_slide:
        if slide_ext is not None:
            slides = [slide for slide in slides if os.path.isfile(os.path.join(source, slide)) and slide.endswith(tuple(slide_ext))]
        else:
            raise ValueError("To skip non-slide, please specify slide extensions!")
    else:
...
parser.add_argument('--custom_downsample', type=float, default=1, 
...
parser.add_argument('--skip_non_slide', default=False, action='store_true', help='filter using slide ext')
parser.add_argument('--slide_ext', nargs="+", help='slide extensions to be recognized, svs/ndpi/tiff by default')
...
if __name__ == '__main__':
    ...
    seg_times, patch_times = seg_and_patch(**directories, **parameters,
                                            ...
                                            skip_non_slide=args.skip_non_slide, slide_ext=args.slide_ext)