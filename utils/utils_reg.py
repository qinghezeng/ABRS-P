...
# -*- coding: utf-8 -*-
"""
Created on Wed Jan 19 15:41:05 2022
...
def collate_MIL(batch): 
	...
	label = torch.vstack([item[1] for item in batch])
	return [img, label]

def collate_MIL_augm(batch):
    import random
...
def generate_split(val_num, test_num, samples, n_splits = 5,
	...
	for i in range(n_splits):
		...
		val_ids = np.random.choice(indices, val_num, replace = False) # validation ids

		remaining_ids = np.setdiff1d(indices, val_ids)
