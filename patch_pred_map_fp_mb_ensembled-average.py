...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(save_dir, f"ensembled-aver_{args.score_type}_maps_{args.k}f_{args.downscale}_{args.norm}")
    ...
    for i in range(total): 
        ...
        if args.cpu:
            pred = torch.load(os.path.join(args.save_dir, f"{args.score_type}_scores_{folds[0]}", f"{args.score_type}_score_"+ # use patch_pred_score_ if to plot patch prediction heatmap
        ...
        for br in brs:
            ...
            df[f'{args.score_type}_score'] = list(arr_pred/args.k)
            os.makedirs(os.path.join(args.save_dir, f"ensembled-aver_{args.score_type}_scores_{args.k}f_{args.norm}"), exist_ok=True)
            df.to_csv(os.path.join(args.save_dir, f"ensembled-aver_{args.score_type}_scores_{args.k}f_{args.norm}", f"{os.path.splitext(patch_bags[i])[0]}_{br}.csv"),
                      index=False)
            nor = [(x - np.min(arr_pred)) / (np.max(arr_pred) - np.min(arr_pred)) for x in arr_pred]
                
            # for the B highest and B lowest, save the original patch named with the pred score and coords
            if args.B > 0:
                patch_dir = os.path.join(args.save_dir, f'repres_patches_{args.score_type}_{args.k}f_br{br}')
